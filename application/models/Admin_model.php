...
class Admin_model extends CI_Model
{
    ...
    public function getPemasukan($range = null  )
    {
        $this->db->select('*');
        $this->db->join('user u', 'pm.user_id = u.id_user');


        if ($range != null) {
            $this->db->where('tanggal_pemasukan' . ' >=', $range['mulai']);
            $this->db->where('tanggal_pemasukan' . ' <=', $range['akhir']);
        }

        $this->db->order_by('id_pemasukan', 'DESC');
        return $this->db->get('pemasukan pm')->result_array();
    }
